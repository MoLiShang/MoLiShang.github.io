document.addEventListener('DOMContentLoaded', function () {
  ...
  function translatePage () {
    if (targetEncoding === 1) {
      currentEncoding = 1
      targetEncoding = 2
      translateButtonObject.getElementsByTagName('i')[0].textContent = msgToTraditionalChinese
      isSnackbar && anzhiyu.snackbarShow(snackbarData.cht_to_chs)
    } else if (targetEncoding === 2) {
      currentEncoding = 2
      targetEncoding = 1
      translateButtonObject.getElementsByTagName('i')[0].textContent = msgToSimplifiedChinese
    ...
    if (translateButtonObject || translateRightMenuButtonObject) {
      if (currentEncoding !== targetEncoding) {
        ...
        if (translateButtonObject) {
          translateButtonObject.getElementsByTagName('i')[0].textContent = textContent;
        }
